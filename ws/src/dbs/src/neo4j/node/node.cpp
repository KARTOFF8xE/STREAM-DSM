    ...
    std::string getPayload(std::string name, u_int64_t handle, pid_t pid) {
        return fmt::format(R"(
            {{ 
                "statements":
                    [
                        {{ "statement": "MERGE (n:Node{{name:$name}}) ON CREATE SET n.handle=$handle, n.state=1, n.stateChangeTime = TIMESTAMP(), n.pid=$pid, n.bootcounter = 1 ON MATCH SET n.handle=$handle, n.state=1, n.stateChangeTime = TIMESTAMP(), n.pid=$pid, n.bootcounter = n.bootcounter+1, n.Services=[] WITH n OPTIONAL MATCH (n)-[r]-() DELETE r WITH n RETURN DISTINCT n ",
                        "parameters": {{
                            "name": "{}",
                            "handle": "{}",
                            "pid": "{}"
