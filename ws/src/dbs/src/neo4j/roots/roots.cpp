    ...
    std::string getPayloadCreateNameSpaceAndLinkPassiveHelpers(std::string fullName) {
        return fmt::format(R"(
            {{
                "statements":
                    [
                        {{ "statement": "WITH $name AS path WITH split(path, \"/\")[1..] AS parts WITH range(0, size(parts)-1) AS indices, parts UNWIND indices AS i WITH \"/\" + reduce(s = \"\", p IN parts[0..i+1] | CASE WHEN s = \"\" THEN p ELSE s + \"/\" + p END) AS folderPath, CASE WHEN i = 0 THEN \"/\" ELSE \"/\" + reduce(s = \"\", p IN parts[0..i] | CASE WHEN s = \"\" THEN p ELSE s + \"/\" + p END) END AS parentPath MERGE (f {{name: folderPath}}) ON CREATE SET f:Passive MERGE (parent {{name: parentPath}}) ON CREATE SET parent:Passive MERGE (parent)-[:namespace]->(f) ",
                        "parameters": {{
                            "name": "{}"
                            }}
                        }}
    ...
    std::string createRoot(std::string hostName, std::string macAdress) {
        return fmt::format(R"(
            {{
                "statements":
                    [
                        {{ "statement": "MERGE (n {{pid: 0}}) SET n:Passive, n.name = $name, n.mac = $mac RETURN DISTINCT {{ id: toInteger(last(split(elementId(n), ':'))) }} ",
                        "parameters": {{
                            "name": "{}",
                            "mac": "{}"
                            }}
