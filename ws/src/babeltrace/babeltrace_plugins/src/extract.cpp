...
ServiceClient extractServiceClientInfoFromEvent(const bt_event *event) {
    ...
    if (bt_field_class_get_type(field_class) == BT_FIELD_CLASS_TYPE_STRUCTURE) {
            ...
            service.name = std::string(bt_field_string_get_value(field));
            field = bt_field_structure_borrow_member_field_by_name_const(payload_field, "node_handle");
            service.node_handle = bt_field_integer_unsigned_get_value(field);
